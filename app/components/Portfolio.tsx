...
export default function Portfolio() {
  return <section className="section portfolio" id="portfolio">
    ...
    <div className="portfolio-note"><div><span className="portfolio-note-label">Also from RM Digital</span><img className="field-force-logo" src="/brand/products/field-force.png" alt="Field Force logo" /></div><p><strong>Field Force</strong> — our sales-performance and insurance-distribution workflow product.</p><a className="text-link" href="https://fieldforce.rmandco.co.za">Explore Field Force <span aria-hidden="true">↗</span></a></div>
  </section>;
}
...
export function RiskLaunch() {
  return <section className="section risk-launch risk-expanded" id="rm-risk">
    <div className="risk-introduction"><p className="section-no">Introducing RM Risk</p><div className="risk-logo-panel"><RiskLogo /></div><span className="launch-status">Launching soon</span><h2>Protecting value.<br />Enabling confidence.</h2><p>Helping organisations identify, assess, mitigate and monitor risk across operations and strategic decision-making.</p><a className="button primary" href="mailto:hello@rmandco.co.za?subject=RM%20Risk%20enquiry">Enquire about RM Risk <span aria-hidden="true">↗</span></a></div>
    <div className="risk-focus"><p className="eyebrow">Core service lines</p><h3 className="risk-services-heading">Expertise for a more resilient tomorrow.</h3><ol className="risk-services-list">{riskServices.map((service, index) => <li key={service}><span>{String(index + 1).padStart(2, "0")}</span><h4>{service}</h4></li>)}</ol><p className="launch-note">Launching soon. Contact us to discuss these planned service lines.</p><p className="risk-values">People · Insight · Resilience · Long-term value</p></div>
  </section>;
}
